...
| 5 | Spawner core: the hub spawns a headless participant when a message mentions it | 📝 | READY | docs/superpowers/plans/m5-spawner-core.md | Grill D2, D5, D7-D9, D17; F1-F5, F8, F10-F11. HIGH. Backend only: exchange state machine, claude/codex spawns (both command lines measured 2026-09-05), hub system row (schema v4), notes, stop + state API, SignalR event, live check script. UI is row 16. |
| 16 | Live exchange UI: working indicators, remaining budget, stop button, concluded marker | [ ] | BLOCKED: M5 | — | Grill D17. Renders `GET /api/rooms/{id}/exchange` + SignalR `ExchangeChanged`; stop = `POST .../exchange/stop`; styles `hub` system notes. opus builder (owner-visible). |
