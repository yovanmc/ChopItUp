...
| 8 | Roster as data: participants become rows named for models, tokens and host configs per row | [ ] | READY | — | Grill D14. Replaces the `TokenStore.Participants` constant; `--print-config` emits one file per row; Codex half enumerable via `codex app-server model/list` (F7). App-backed rows optional. Prerequisite for M5. |
