...
|---|-------|--------|-------|------|-------|
| 10 | Centralised memory: one store both vendors' agents read; agents propose, the owner approves in the room | ✅ | DONE | — | Merged `e838645` (PR #32) 2026-09-06: `MemoryStore` + lazy git trail, schema v5 `memory_proposals`, `recall`/`propose_memory`, approval API + `MemoryPanel`, Claude/Codex import. Live check 19/19 (real Sonnet), dry run 24/24, self-check 27/27, screenshot + UIA gates PASS; deployed, live DB migrated to v5 (`.v4.` backup beside it). Unverified: real Codex memory shape. |
