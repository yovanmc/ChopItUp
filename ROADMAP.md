...
| 5 | Autonomous turns: the hub spawns a headless participant when a message mentions it | [ ] | BACKLOG | — | Both spawn contracts verified end-to-end 2026-09-05: `claude -p` and `codex exec` each posted to the live hub headless, subscription auth, no API keys. Needs a plan + `dissect` before build. Traps: docs/LESSONS.md. |
