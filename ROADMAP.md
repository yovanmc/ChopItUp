...
|---|-------|--------|-------|------|-------|
| 32 | Two human prompts in one room run two exchanges side by side | ✅ | DONE | — | Merged `d258f8d` (#90), deployed (self-check 27/27, live GET carries `exchanges`). Supersede by mention overlap; per-exchange stop API; the room Stop ends every exchange until row 34. 945 tests, 9/9 mutations killed, CI passed. Live two-model check not run (spend). |
...
| 34 | One exchange bar per exchange, each with its own stop | 📝 | READY | [brief](.scratch/m34-exchange-bars/brief.md) | Row 32's UI half. Until it ships the one Stop ends every exchange in the room and the bar shows the newest open one. |
