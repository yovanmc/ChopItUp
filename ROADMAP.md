...
| 23 | Memory v1.1b: consolidation pass — `rewrite` proposals reviewed as a diff | ✅ | DONE | — | Merged `034167a` (PR #59); deployed 2026-09-09, `/health` schema 9 (no migration). Rollback exe: `C:\Self Apps\ChopItUp.backup-20260909-033551`. 731 tests green, dry run 21/21, deploy self-check 27/27, diff card gated live. Review caught a backup-copy path that could write with no backup. |
| 25 | No agent can install a skill after a deploy: import is CLI-only | [ ] | BACKLOG | — | Owner half DONE 2026-09-09: `consolidate-memory` verified live via `/api/skills`, closing row 23's last deployed gap. Remaining is the fix itself: `C:\Self Apps\...\data` writes are guard-denied and `SkillsApi` is GET-only, so every future skill needs the owner. Give the hub its own import path. BACKLOG so it cannot preempt row 24. |
| 24 | Memory v1.1c: Claude Code-shape vendor export (`--export-memory`) | 📝 | READY | docs/superpowers/plans/m24-memory-v1-1c-export.md | Ledger item 8; HIGH (delete/replace aimed at the owner's real memory dir). Critiqued: pass 1 (fable) 5.8, pass 2 (opus) 6.7 — pass 2 caught two pass-1 fixes that each added a fresh data-loss defect. Preflight PASS 22/2. Deletes the findings ledger at close-out. |
