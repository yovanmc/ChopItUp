...
| 25 | No agent can install a skill after a deploy: import is CLI-only | [ ] | OWNER: `--import-skill tools\skills\consolidate-memory` into the live hub's data dir | — | Guard-deny 2026-09-09: agent writes under `C:\Self Apps\...\data` are denied, so row 23's skill reached the repo and dev but not the live hub. Paired fix: `SkillsApi` is GET-only — give the hub its own import path. |
