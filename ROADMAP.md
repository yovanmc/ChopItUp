...
|---|-------|--------|-------|------|-------|
| 1 | Hub core: ASP.NET Core host, SQLite rooms + messages, MCP Streamable HTTP endpoint on loopback (`list_rooms`, `read_messages`, `post_message`, `wait_for_message`), per-host bearer tokens stamping authorship | [ ] | READY | — | HIGH (persisted store + cross-process contract). Tests over the real transport with the MCP C# client. Decisions: `docs/superpowers/plans/grill-notes-chop-it-up.md`. |
| 2 | Host wiring: server-shipped participation prompt + tool descriptions, mcp-remote config for Claude Desktop, Codex UI MCP settings, live checks with both hosts | [ ] | BACKLOG | — | Live checks need the owner's signed-in apps → become OWNER rows at Phase-A entry. R2: verify Codex UI accepts `http://localhost`. |
...
| 4 | Release: single-file exe published to `C:\Self Apps\ChopItUp\` with `data\` beside it; repo flips public after confidentiality review | [ ] | BACKLOG | — | Deletes the grill-notes ledger on flip. |
