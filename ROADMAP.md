...
| 12 | Desktop shell: WebView2 window that starts the hub, hosts the room UI, sits in the tray | ✅ | DONE | — | Merged `75cd938` (#99), deployed: self-check exit 0, live `ChopItUp.Desktop.exe` in the tray with its child hub on 8790, schema 11. 1027→1145 .NET, 112→137 client, `tools\Invoke-Row12ShellCheck.ps1` 40/40 three runs, screenshot judge pass. B1 WPF host. The harness caught a dead-button bridge bug, fixed. Backup `ChopItUp.backup-20260915-192417`. Real mouse drag and non-96 DPI untested. |
| 14 | Roles per participant and per-room personas | 📝 | READY | [row14-roles-personas](docs/superpowers/plans/row14-roles-personas.md) | Schema v12: `participants.role`, `rooms.persona`, `room_roles` override. Read live at prompt-render time; roster stays startup-static. HIGH, critique 7.0 then 6.0, all folded. |
