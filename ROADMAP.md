...
| 4 | Release: single-file exe published to `C:\Self Apps\ChopItUp\` with `data\` beside it | [ ] | READY | — | `IncludeNativeLibrariesForSelfExtract` (e_sqlite3.dll; `AppContext.BaseDirectory` stays beside the exe). **Repo stays private — the public flip is the owner's explicit yes, not this milestone's.** Deletes grill notes. |
