...
| 34 | One exchange bar per exchange, each with its own stop | ✅ | DONE | — | Merged `095e9c7` (#92), deployed (self-check 27/27, served client carries it). Scratch-hub gate with a stub `codex.cmd`: two strips, buttons hit-test, per-root POST, socket render. No authenticated in-page click (token injection denied). |
| 35 | A git worktree per exchange in a directory room, auto-merged on conclude | 📝 | READY | [plan](docs/superpowers/plans/row35-exchange-worktrees.md) | Owner ruling 09-13: both exchanges really edit at once; a conflict leaves the branch and posts a note naming it. Until then a directory room runs one spawn at a time. |
