...
| 30 | Four live-check legs lost their cheap spawnable-row bearer | 📝 | READY | .scratch/m30-live-check-bearers/brief.md | LOW. `Invoke-M18MemoryCheck` sends 3 `propose_memory` calls as bearerless `opus`: 5/8, then a throw leaves 8 of 16 checks unrun. `Invoke-M25SkillProposalCheck` 36/37, `ac3.no-second-card` passing by coincidence [V 2026-09-10 96ef837]. Move to `claude`, no spawn. |
| 33 | The M9 room check posts to `/api` with no bearer | [ ] | READY | — | `Invoke-Api` (`tools/Invoke-M9RoomCheck.ps1:57`) sends no bearer, `BearerTokenMiddleware.RequiresAuth` (`:40`) demands one on non-GET `/api`, and the first call is a POST (`:76`) [V 2026-09-10 96ef837]. inferred: aborts before any spawn. Seed an owner bearer like the other harnesses. |
