...
| 35 | A git worktree per exchange in a directory room, auto-merged on conclude | ✅ | DONE | — | Merged `ebde29a` (#94), deployed (self-check 27/27, live /health schema 10). 945→993 tests; stub-CLI dry run 4/4 (concurrent merge, conflict keep, stop keep, crash recovery). Live Claude/Codex spawn in a worktree not run (spend). |
| 36 | Reply-to in the UI picks which exchange a post joins | 📝 | READY | docs/superpowers/plans/row36-reply-to-joins.md | HIGH. Schema v11 `messages.reply_to_id`; a reply joins its exchange (reopening a closed idle one, turns and skill kept, a kept worktree branch continued). Class B defaults B1-B10 in the plan. Critique fable 6.9, opus 7.0, both folded. |
