...
| 13 | Symmetric confinement: run Claude spawns under a restricted Windows account | [ ] | BACKLOG | — | Grill D13, F5. Only route to OS-level confinement on native Windows. Measured: Codex 0.153.3 `--approve-for-me` did not stop an in-workspace `git commit` (09-06); the absolute-path deny binds on Claude spawns (09-07, M11) — Codex is the open side. M9 residuals (no-auth `/api`, `tokens.json`, absolute-path git) land here. |
