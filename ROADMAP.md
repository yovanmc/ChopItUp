...
| 37 | Verification debt: row 35 live check, repo verify skill, Roles dialog look | 📝 | READY | `.scratch/m37-verification-debt/brief.md` (lite path, LOW) | Owner pick 2026-09-17 over a memory editing page, spawn confinement (D13) and a live Claude Desktop participant. Proves exchange worktrees with one real Claude and one real Codex spawn (row 35 left Codex in a `.git`-file worktree unverified), creates the gitignored `.claude/skills/verify-chopitup/` from the workflow template and graduates it on the Roles dialog, whose capture gets an opus look. Spend ≤ 4 model calls. Baseline at 10d7875: 1208 dotnet + 162 vitest green. |
