...
|---|-------|--------|-------|------|-------|
| 40 | Memory editing page | ✅ | DONE | — | DONE 2026-09-17 (#110, main 5f858d3; deployed, hub schema 12, backup `ChopItUp.backup-20260917-170635`). `Edit memory` dialog; `GET/PUT /api/memory/topics/{slug}` + `preview`; a save is an approved rewrite (`.bak`, commit, note) via the shared `ApproveCore`. .NET 1,219, vitest 188, `Invoke-Row40MemoryEditCheck.ps1` 17/17, capture 25/25, opus look PASS. Unverified: soft wrap. |
| 41 | Memory editor polish | [ ] | BACKLOG | — | From the row 40 look and review: `room-*` topics show the 24,000 cap while a spawn reads 2,000 (`SpawnerService.cs:961`); after a save the count line sits 11 chars above the picker (placeholder proposal id in the preview); the card's top edge shows dimmed header glyphs; hint `Saving files an approved rewrite` reads as a typo; soft wrap unmeasured. |
