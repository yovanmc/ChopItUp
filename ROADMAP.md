...
| 21 | Runs runbook: `docs/verification.md` gains an H3 "Timeouts inside a run" (the row 20 dogfood target) | [ ] | BLOCKED: reserved for the row 20 dogfood run | — | LOW. Built by the room run from the phone, never by a harness session. Acceptance: the H3 (at most 8 lines) states the 25-min gate ceiling, the 30-s progress cadence and the CLI's 300-s cut it answers, and names `tools\Probe-McpTimeouts.ps1` as the proof. |
