...
|---|-------|--------|-------|------|-------|
| 16 | Live exchange UI: working indicators, remaining budget, stop button, concluded marker | ✅ | DONE | — | Merged `5c6fa73` (PR #29) 2026-09-05: `ExchangeBar` (chips, turns left, Stop, outcome marker), hub notes as system rows, `seq`-guarded snapshot. Gated vs real Sonnet spawns (open/stop/concluded live, judge PASS); self-check 27/27; deployed, hub restarted. Unverified: room switch, 409 banner. |
| 10 | Centralised memory: one store both vendors' agents read; agents propose, the owner approves in the room | 📝 | READY | docs/superpowers/plans/m10-centralised-memory.md | Grill D15, F9–F10. HIGH, 8 tasks, schema v5; critiqued twice (opus 6.6, fable 7.1), all folded. Core in the spawn prompt, `recall` + `propose_memory` tools, approval panel, vendor importers. |
