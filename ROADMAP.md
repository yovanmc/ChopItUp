...
|---|-------|--------|-------|------|-------|
| 22 | Run strip: the stop the owner can actually reach (D2's fourth stop) | ✅ | DONE | — | Merged `9a7a43a` (PR #68); deployed 09-09, self-check 27/27. **Rollback: `C:\Self Apps\ChopItUp.backup-20260909-213116`**. Client 36 → 51; .NET 867 unchanged. Gate clicked `active` and `parked` runs with nothing open and nothing in flight — zero buttons there before — to `ended`. |
| 27 | A cap-driven park tells the owner they stopped it | 📝 | READY | .scratch/m27-cap-park-attribution/brief.md | `ExchangePolicy.Stop` hardcodes "stopped by the owner" but `ParkRun` calls it on the wall-clock/spawn/phase caps; `OUTCOME.stopped` then renders "Stopped by you" [V 2026-09-09 8737f1b]. Split out of row 22 to keep that row UI-only. |
