...
| 29 | An owner-class credential sits outside the data dir and no read fence can reach it | [ ] | READY | — | Claims re-verified [V 2026-09-10 c971b58]: `owner-remote` is kind `human`, only a human row starts runs, its bearer is pasted into a `.mcp.json` (`HostConfigs.cs:222`). Fencing is dead on BOTH CLIs — Claude's read fence was already measured ineffective, and Codex's sandbox confines writes only (measured 09-10, LESSONS "M29 scoping"). Title's "symmetric confinement" dropped: rescope to shrinking the prize. Which lever is undecided. Row 13's residual. |
