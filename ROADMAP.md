...
| 37 | Verification debt: row 35 live check, repo verify skill, Roles dialog look | 📝 | READY | `.scratch/m37-verification-debt/brief.md` | Lite path, LOW. Owner pick 2026-09-17 over a memory editing page, spawn confinement (D13) and a live Claude Desktop participant. One real Claude and one real Codex spawn prove exchange worktrees (row 35 left Codex in a `.git`-file worktree unverified); the gitignored verify skill graduates on the Roles dialog, whose capture gets an opus look. Spend ≤ 4 calls. |
