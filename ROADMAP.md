...
| 26 | Probe: does a running Claude Code session read an exported memory directory | 📝 | READY | .scratch/m26-memory-probe/brief.md | Lite brief. [V 2026-09-16 this session] `--bare` skips auto-memory, so the probe runs `-p` with `--settings` carrying `autoMemoryDirectory`; index cap 200 lines/25 KB; topic files read on demand. The exporter refuses at 199 live entries, so the 199 leg hand-appends one index line. Spends at most 4 Sonnet calls. Carried out of row 24 (`f964ece`). |
