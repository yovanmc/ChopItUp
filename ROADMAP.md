...
|---|-------|--------|-------|------|-------|
| 29 | An owner-class credential presented from inside a spawn is refused | ✅ | DONE | — | Merged `5ad8c57` (#80); deployed 09-10. **Rollback: `C:\Self Apps\ChopItUp.backup-20260910-212028`**. 930 green (896 → 930), live check 10/10, deployed probes clean; owner-token legs → row 31. Three in-flight changes #80 explains: zero live jobs now ALLOW without a lookup (`9da479b`, fail-closed there was a pure lockout), the `Host` gate parses the address instead of matching a list (`5fce1ee`, so no `Host` header at all is now refused), and plan claim 20 was wrong — a `cmd` shim's `conhost` sits OUTSIDE the job ~10/15, the named worker inside 15/15. |
...
| 17 | Spawner test A5 races its own HTTP round trip | 📝 | READY | [brief](.scratch/m17-a5-race/brief.md) | TEST defect [V 2026-09-10 e4210d3]: 6/55 under pinned-CPU load, 0/15 solo (`32847a8`). A5 reads `Pending` right after a real MCP round trip, and its 500 ms no-spawn check can pass before the mark lands. Fix: poll for the mark first, plus a direct policy test (`Accept` is private). |
