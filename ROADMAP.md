...
|---|-------|--------|-------|------|-------|
| 30 | Four live-check legs lost their cheap spawnable-row bearer | ✅ | DONE | — | Merged `e5e282c` (#85); `tools/` only, deploy unchanged. The `opus` MCP calls now run as `claude`. M18 5/8+abort → 16/16, no `SKIPPED`. M25 36/37 → 37/37; `ac3.no-second-card` now needs the first offer to succeed (mutation run failed both ac3 checks). 931 tests green, PR CI passed. |
...
| 33 | The M9 room check posts to `/api` with no bearer | 📝 | READY | .scratch/m33-m9-check-bearer/brief.md | LOW. Re-verified at adf5a95. `Invoke-Api` (`tools/Invoke-M9RoomCheck.ps1:57`) sends no bearer, `BearerTokenMiddleware.RequiresAuth` (`:40`) demands one on non-GET `/api`, and the first call is a POST (`:76`) [V 2026-09-10 96ef837]. inferred: aborts before any spawn. Seed an owner bearer like the other harnesses. |
| 32 | Two human prompts in one room run two exchanges side by side | [ ] | READY | — | Owner ask 09-10 while picking row 29's lever: "tell Claude task A and Codex task B in separate prompts, both running". Today a second human post in a room supersedes the first (`ExchangePolicy.OnMessage`, D5); fan-out works only across rooms. Promoted 09-13. |
