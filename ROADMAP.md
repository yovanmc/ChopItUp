...
| 28 | A spawn can act as the owner | 📝 | READY | .scratch/m28-spawn-owner-escalation/plan.md | Directory/run spawns get Bash; every `/api` route but `/mcp` and the two skill-decision POSTs is unauthenticated, so a spawn can POST a message stamped `owner` with no credential. `data	okens.json` also holds every token in the clear [V 2026-09-09 7ef0461]. An owner post starts runs. ACL/DPAPI are dead ends (same user). Replaces row 13. |
