...
| 25 | No agent can install a skill after a deploy: import is CLI-only | [ ] | BACKLOG | — | Owner half DONE 2026-09-09 (`consolidate-memory` verified live via `/api/skills`). The fix remains: `C:\Self Apps\...\data` writes are guard-denied and `SkillsApi` is GET-only, so every future skill needs the owner. Give the hub its own import path. Its "cannot preempt row 24" hold expired at `f964ece`. |
