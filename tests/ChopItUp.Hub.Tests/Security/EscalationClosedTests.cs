...
/// <summary>The threat model itself, as a test. Starts from what a spawned process can actually do
/// (read every file under the hub's data directory, and make loopback HTTP calls) and ends at an
/// owner-attributed write being unreachable. Deliberately ONE test: a suite of narrow unit facts could
/// each pass while the combination the escalation lives in still works.
///
...
/// never held it, is covered by <see cref="Security.OwnerPeerCheckEndToEndTests"/>: a real child
/// process inside a hub-owned job, holding a live <c>owner-remote</c> bearer, is refused over both
/// loopback families, while the same command run outside every job is accepted.</summary>
public sealed class EscalationClosedTests : IAsyncLifetime
{
    ...
    public async Task A_process_that_reads_the_whole_data_dir_and_calls_loopback_http_can_never_forge_an_owner_write()
    {
        ...
        // 1) A no-credential POST is refused. Without this, narrowing auth back down to only /mcp
        // and the two skills-decision routes would leave every assertion below green, and this test
        // would prove nothing about the door it exists to guard.
        ...
        // has already started (the start-time sweep of host-configs\ only runs once, at start:
        ...
        // 4) The residual, made concrete: data\spawns\<id>\mcp.json legitimately holds a live MODEL
        // bearer while that spawn runs (SpawnerService.Launch writes exactly this shape). This plants
        // that real shape with a genuinely live ephemeral token, so the walk below exercises it: a
        // candidate found here MUST resolve (/mcp accepts it) but MUST NOT authenticate an
        // owner-attributed write. Absence would be the wrong claim for this one; refusal is the right one.
        ...
        // 5) Walk the WHOLE data dir, not just tokens.json and host-configs\. memory\core.md and
        // memory\.gitignore (MemoryStore.EnsureLayout) exist by construction of every hub start, so
        // this assertion fails on its own if the enumeration below is ever narrowed back to the two
        // known locations.
        ...
        foreach (var file in allFiles)
        {
            ...
            foreach (var candidate in TokenScan.Candidates(text).Distinct(StringComparer.Ordinal))
            {
                ...
                // /mcp), but the /api write guard must still refuse it here.
