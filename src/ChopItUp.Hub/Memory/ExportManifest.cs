...
/// <summary>The states <see cref="ExportManifest.Verify"/> distinguishes: nothing there, something
/// there with no record at all, a record that cannot be parsed, a record bound to a different store,
/// a record bound to this store whose files no longer match it, and a clean match.
/// <see cref="Foreign"/> and <see cref="Absent"/> are never the same thing: a populated directory
/// with no manifest must refuse, not proceed as if empty.</summary>
public enum TargetState { Absent, Foreign, Unreadable, DifferentSource, Drifted, Clean }

/// <summary><see cref="Paths"/> is the affected-paths list every refusal and every override share
/// verbatim; <see cref="ManifestRoot"/> is the manifest's own <c>SourceRoot</c>, carried so a
/// <see cref="TargetState.DifferentSource"/> report can print both roots without re-reading the
/// manifest.</summary>
...
/// root path and only that, never by content: two stores holding the same entries are
/// indistinguishable by content (so a scratch store could pose as the real one), and a store's
/// content changes on every legitimate re-export. <see cref="SourceFingerprint"/> is carried purely
/// for the report to say whether anything changed since last time; it is never a refusal predicate,
/// and must never be promoted into one. <see cref="Files"/> maps a recursive path (relative to the
/// target directory, forward-slash separated) to that file's SHA-256 hex hash; the manifest's own
/// file (<see cref="FileName"/>) is deliberately excluded from this map and from every enumeration
/// <see cref="Verify"/> performs: it cannot hash itself, and a naive recursive diff would flag it as
/// drift on every second run.</summary>
    ...
    IReadOnlyDictionary<string, string> Files)
{
    ...
    /// never produced, must still read; that permissiveness, not per-version branching, is the
    /// schema-evolution guard.</summary>
    ...
    /// <see cref="Version"/> newer than <see cref="CurrentVersion"/>) and never throws. The caller
    /// decides what an unreadable record means.</summary>
    ...
    /// <summary>Report-only: SHA-256 hex over <c>"topic\ntitle\nbody\n"</c> per live entry, in the
    /// same render order <see cref="MemoryExport.Render"/> uses (core first, then
    ...
    /// construction, and an ordinary store's fingerprint changes on every legitimate re-export. Only
    /// <see cref="SourceRoot"/> distinguishes stores.</summary>
    ...
    /// Order matters: a source mismatch is decided before any file comparison, so a matching set of
    /// file hashes can never satisfy the guard on its own; <paramref name="store"/> is consulted only
    /// for its <see cref="MemoryStore.Root"/>, never re-rendered to compare content.</summary>
    public static ManifestVerdict Verify(ExportManifest? manifest, string targetDir, MemoryStore store)
    {
        ...
        if (manifest is null)
        {
            ...
            return manifestFileExists
                ...
                : new ManifestVerdict(TargetState.Foreign, otherPaths, null);   // populated + no manifest refuses
        }

        // Computed regardless of the source check below: DifferentSource must still be able to print
        // a drift list, not just both roots.
    ...
    /// <summary><c>OrdinalIgnoreCase</c> over the trimmed, fully-qualified path.</summary>
    ...
    /// to it, excluding the manifest's own file.</summary>
