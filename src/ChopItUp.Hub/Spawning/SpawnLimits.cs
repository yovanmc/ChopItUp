...
/// <summary>The caps are hard code, not configuration. <see cref="Default"/> is the only
/// instance the hub ever constructs; tests build smaller ones through the DI seam in
/// <c>HubHost.Build</c>. Budget = model turns per exchange (8; a turns: token in the
/// leading run overrides it up to ExchangeCommands.MaxTurns); Debounce = how long after the
/// last triggering message the hub waits before launching, so one burst is one spawn;
/// MinSpacing = gap between two launches of the same participant, across rooms;
/// Timeout = wall clock per plain-room spawn, then the tree is killed; OutsideDirectoryTimeout
/// extends only directory-room spawns outside a run. Test limits that omit it keep their short
/// Timeout in both room shapes. Transcript* = the prompt window.</summary>
