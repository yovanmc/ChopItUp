...
public sealed class MemoryTools(MemoryStore memory, MemoryProposalStore proposals, MessageStore store, MessageSignal signal, IHttpContextAccessor http, ParticipantStore participants)
{
    ...
    [McpServerTool(Name = "recall", ReadOnly = true, Idempotent = true, OpenWorld = false),
     Description("Read the shared memory every participant uses. With nothing: the core (what every model should know) and every topic with its entry titles. With a topic: that topic's text. With a query: every entry whose title or body contains it, across all topics or within the given one, at most 50 hits with capped: true when the limit was reached. The core in the no-argument reply is cut at 6000 characters and says so (truncated: true). Use it when a question about the owner or their work needs memory your context does not already carry, and before proposing a memory, to check the fact is not already there. An unknown topic with no query is refused with the list of topics that exist, and with a query it returns no hits.")]
    ...
    [McpServerTool(Name = "propose_memory", ReadOnly = false, Destructive = false, Idempotent = false, OpenWorld = false),
     Description("Propose one durable fact for the shared memory: something about the owner or the work that memory does not already say and that every future model should know. The hub records you as the proposer and announces it in the room. The owner approves or rejects it there. Nothing is remembered until approved. Propose once per fact, never per message. Returns the proposal (id, status, kind, replaces, and any flags the hub raised on the body). A proposal whose topic and title match one still pending returns that pending one (id, topic, title, status) with duplicate: true instead of a new one. A title the topic already holds is refused unless replaces names it, and a replaces title the topic does not hold is refused with the titles it does hold.")]
    ...
    [McpServerTool(Name = "propose_rewrite", ReadOnly = false, Destructive = false, Idempotent = false, OpenWorld = false),
     Description("Propose a consolidated version of ONE memory topic: the whole file, rewritten. Read the topic with recall first. Fold duplicates and contradictions, keep every distinct fact, invent nothing, keep one '## ' heading per entry, and keep a surviving entry's heading exactly as it was. The owner sees it as a diff against the current file, with the entries it would remove named, and approves or rejects it. Nothing changes until then. Use propose_memory for a single new fact, because this replaces everything in the topic. Only human rows and Claude-hosted model rows may call it. Refused with a reason when the topic has no file, when the topic is longer than recall can return whole (24000 characters), when the resulting file would pass the cap (6000 characters for core, 24000 for any other topic), or when a rewrite of the topic is already pending. Returns the proposal in propose_memory's shape.")]
