...
/// <summary>Guards <c>/mcp</c> (any resolvable participant) and every <c>/api</c> request whose
/// method is not GET/HEAD/OPTIONS: a valid bearer token sets <see cref="ParticipantKey"/> in
/// <see cref="HttpContext.Items"/>; anything else is 401 before any MCP or handler code runs. An
/// owner-attributed write must not be forgeable by anything that can merely reach the loopback
/// address. <c>GET</c>/<c>HEAD</c>/<c>OPTIONS</c> on <c>/api</c> stay unauthenticated.
...
/// is let through, and anything else that resolves is 403, not 401.</summary>
public sealed class BearerTokenMiddleware(RequestDelegate next, TokenStore tokens, IOwnerPeerCheck peers, MessageStore store, MessageSignal signal)
{
    ...
    /// <summary>The fixed body for an owner credential presented from inside a spawn.</summary>
    public const string InsideSpawnError = "owner credential refused: presented from inside a spawn";

    /// <summary>The fixed prefix for an unresolvable peer; the reason is appended in parentheses.</summary>
    public const string UnresolvableErrorPrefix = "owner credential refused: peer process unresolvable";

    /// <summary>The hub's owner-class pair, shared by the peer check and the /api non-GET branch so both
    /// agree on what "owner-class" means.</summary>
    ...
    /// prefix match (an MCP session may legitimately request any sub-path under it); every other
    /// <c>/api</c> request needs a credential unless its method is GET, HEAD or OPTIONS: a blanket
    /// rule by method, not a route list, so a new write endpoint is guarded by construction.</summary>
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        {
            // The hub's owner-peer check runs right after a bearer resolves and before the /api-only branch
            // below, on /mcp as well as /api: the phone session's owner-remote bearer posts through
            // /mcp, so the check must run there too.
    ...
    /// <summary>Hub-owned text, posted through the same <c>MessageStore.Post</c> +
    /// <c>MessageSignal.Publish</c> pair <c>RunTools.PostNote</c> uses, as <see cref="ChopDb.HubParticipantId"/>.
    /// A posting failure never blocks the refusal itself: it is logged and swallowed, like every
    /// other hub note.</summary>
