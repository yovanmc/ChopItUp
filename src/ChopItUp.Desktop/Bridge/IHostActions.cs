...
/// <summary>Everything the hosted page is allowed to ask the window for. <c>MainWindow</c>
/// implements it; <c>HostBridge</c> is a pure function from a JSON message onto these six members,
/// which is what lets the whole wire protocol be tested without a window.
///
...
public interface IHostActions
{
    ...
    /// attached hub may be on a port that is not the one the shell was asked for.</summary>
    HubStatus Hub { get; }
    ...
    /// <summary>Hides to the tray. The hub keeps running and the tray icon stays; this is not a
    /// quit, and the window comes back with its bounds unchanged.</summary>
    void Close();

    /// <summary>Stops the hub this shell started and exits. Kills, does not ask.</summary>
    void Quit();
}
