...
public sealed class ParticipantStore(ChopDb db)
{
    ...
    /// <summary>The hub owner's row. Kind 'human' is not unique (the remote proxy is human too), so this
    /// is an id lookup, not a kind filter: `owner` is the identity every owner-scoped read resolves to
    /// (unread counts, the web UI's post author, the trail identity), while `owner-remote` is a second
    /// hand on the same authority, distinguished in the transcript. Throws when the row is absent,
    /// which no migrated or fresh database can be.</summary>
    ...
    /// dropped, known ones deduplicated and reordered to <see cref="ParticipantClasses.All"/>'s order,
    /// and an empty or whitespace input clears the row (stored as NULL, the same as a row that never
    /// had a class). Returns whether the row existed (exactly one row updated); it never inserts, so an
    /// unknown id changes nothing and returns false.</summary>
    ...
    /// <summary>The cap on a global role and on a room override, enforced here so a future writer that
    /// is not the HTTP API cannot exceed it. Equals <see cref="ChopItUp.Core.Memory.MemoryStore.RoomChars"/>
    /// so the two per-room text budgets match.</summary>
    public const int MaxRoleChars = 2000;

    /// <summary>Sets a participant's global role. The WHERE clause is the spawnability arbiter:
    /// <c>kind = 'model' AND model IS NOT NULL</c>, never <c>kind = 'model'</c> alone, because
    /// <c>claude</c> and <c>codex</c> are kind 'model' with a NULL model, app-backed windows the hub
    /// never spawns, so a role stored there could never render. A non-spawnable or unknown id changes
    /// nothing and returns false without a second query. An empty or whitespace
    ...
    /// <summary>Removes this room's override for a participant, falling back to the global role.
    /// Returns true when the room and a spawnable participant both exist, even when no override row
    /// was there to delete: clearing an absent override is a success, not a failure.</summary>
    ...
    /// <summary>Stores this room's override for a participant, upserting rather than throwing on a
    /// second write. <paramref name="role"/> is trimmed first, like <see cref="SetRole"/> and
    /// <see cref="MessageStore.SetPersona"/>; a whitespace-only input collapses onto the "no role in
    /// this room" sentinel, the empty string stored as a row distinct from having no override at all,
    /// which must not be turned into a delete (use <see cref="ClearRoomRole"/> for that). Returns
    /// false when the room or a spawnable participant does not exist; <c>ChopDb.Open</c> enforces the
    /// foreign keys regardless, so this existence check exists to produce a clean refusal, not to
    /// guard integrity.</summary>
    ...
    /// <summary>This participant's global role, or null.</summary>
    ...
    /// is different from a stored empty string, the "no role in this room" sentinel).</summary>
    ...
    /// room's override when one is stored (including the empty-string sentinel), else the global
    /// role, else null. The deciding function for which role renders, so it is tested directly.</summary>
    ...
    /// <summary>Every override stored in one room, for the Roles dialog.</summary>
