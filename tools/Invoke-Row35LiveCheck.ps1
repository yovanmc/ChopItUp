<#
.SYNOPSIS
    Proves exchange worktrees end to end, with a real Claude spawn and a real Codex spawn, against a
    scratch hub and a scratch directory room.

.DESCRIPTION
    The unit suite stubs the CLI, so only a live run proves that a real `claude`/`codex` process,
    spawned into a linked worktree whose `.git` is a FILE, can create a file there and have the hub
    merge it onto the room's default branch and clean up afterward. This script covers one Claude leg
    (`@sonnet`) and one Codex leg (`@gpt-5.6-terra`, the model the account's Codex config supports -
    `gpt-5.4-mini` measures a `400 invalid_request_error` from Codex, not a hub or worktree defect,
    see docs/verification.md), run sequentially in the same scratch directory room (the hub closes
    one worktree per room at a time).

    Each leg posts one owner message mentioning the participant and asking for ONE small file with
    fixed content - no other tool use is requested, so a one-line reply concludes in about 10 s. It
    then asserts, with git run directly against the room directory:
    ...
    A FAIL on the file-presence check names only the leg (never a message body): a spawn's file
    missing from the default branch after the merge note is a real, nameable outcome, not a script
    defect to chase.

    `-SeedOnly` spends no model call: it starts the hub, binds the room, confirms `/health` reports
    schema 15 and that the hub itself ran `git init` in the room directory (RoomDirectories), then
    stops. Nothing is posted and no exchange ever opens. `-SkipClaude` / `-SkipCodex` each drop
    ...
    same as any other live check, before it is called a defect.
    ...
    depth. CI has neither CLI on PATH: this script is agent-run only, never automatic.

.NOTES
    Every check prints PASS/FAIL; the last line is "Results: n/m PASS"; exit 0 only when every check
    passed. `claude auth status` must report `loggedIn: true` before a full run; this script does not
    check that itself.
#>
[CmdletBinding()]
param(
...
# Gate-script convention (Invoke-M20RoadmapCheck.ps1 carries the same setting): read native exit codes
# from $LASTEXITCODE explicitly rather than letting a nonzero git exit throw, since every git assertion
...
# `git worktree list --porcelain` lines a plain worktree the hub owns as "worktree <path>"; match on
# the leaf folder name ("x<root>") rather than the whole path so backslash/forward-slash spelling
# never matters.
...
# Everything carrying our data dir on its command line except the hub and this script itself: a
# caller who passed -DataDir has it on pwsh's own command line, excluded by $PID.
...
# the hub's close note and classify it, then assert what's left on disk. Never retries: whether to
# re-run a FAIL once before calling it a defect is the caller's decision, not this script's.
function Invoke-ExchangeLeg {
    ...
    # While open, the worktree is registered under <roomDir>.worktrees for the life of the exchange.
    # Poll for up to 90 s: real spawn latency is about 10 s for a one-line ask, so this window is
    # generous, not tight.
...
try {
    ...
    Add-Check -Name 'hub.health-schema' -Passed ($health.schema -eq 15) -Detail "schema=$($health.schema)"
    ...
    # already a repository; a plain repo's `.git` is a folder (only a worktree's is a file).
    ...
    # Stop through the hub first so it kills its own spawned children; only then the hub by PID, then
    # sweep anything that survived either step.
    if ($roomId) {
