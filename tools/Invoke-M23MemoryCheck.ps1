<#
.SYNOPSIS
    Self-check: proves the consolidate-memory skill is installed and reads clean, the schema is still
    what it was, and names the data directory it actually read. Run against a scratch stand-in first,
    then against the deployed build after import.

.DESCRIPTION
    Every check asserts a real .NET boolean or an explicit process exit code, never a string a later
    branch has to re-interpret: a shape-checking wrapper that can record the literal string 'FAIL' as
    a passing check would make every automated row unconditionally green.
    ...
    $env:TEMP. Pointed at the deployed exe and its real data directory, it is the same check, run for
    real.
#>
[CmdletBinding()]
param(
    ...
    [int]$ExpectedSchema = 13,          # pass 999 to force that row RED on demand
...
# stores, so this line is what lets a human catch that mismatch later.
...
$hub = $null
try {
    # Import with the hub stopped (docs/verification.md's runbook line for this skill). Every path
    # element is wrapped in its own literal double quotes: Start-Process -ArgumentList joins its array
    # with plain spaces rather than using ProcessStartInfo.ArgumentList, so an unquoted path containing
    # a space gets word-split by the child's argv parser (--import-skill would receive only
    # "C:\Agent" from "C:\Agent Projects\ChopItUp").
    ...
    # /api/skills already omits anything Tampered or Missing (SkillStore.List skips both), so the
    # skill's mere presence here IS "reads Ok, not Tampered": there is no separate status field to
    # read. Enumerate the array before filtering.
