...
| D4 | Budgets | Exchange budget stays 4 turns; skill phases are cut to fit (build step 1 turn, critique 2, consensus round up to 4). Runs get their own hard-coded caps beside `SpawnLimits.Default`: spawn timeout inside a run, wall-clock per run, spawns per run. Proposed 30 min / 8 h / 80 (numbers ASSUMED, model-originated; the cost-ceiling decision may lower them). Plain exchanges keep D5 and D7 untouched; no skill-declared budgets, no global raise. |
