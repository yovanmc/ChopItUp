...
| 12 | Eleven scripts under `tools/` pin schema 9 and must all be swept — ten as the literal `schema -eq 9`, plus `Invoke-M23MemoryCheck.ps1:32`'s `[int]$ExpectedSchema = 9` (pass 2 finding 5). Task 3's AC3 sweep is now done | a11d23f | — (baseline; the sweep this claim demanded landed this session — all eleven now assert 10, not 9, so a live recheck of the `-eq 9`/`ExpectedSchema = 9` pattern would hard-fail by design; see claim 2) |
