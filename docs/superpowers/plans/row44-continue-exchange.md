...
## Interrogation and review dispositions (Phase B)

Diff interrogation (opus, 7.2, FIX-THEN-SHIP), branch code review (Standards: 3 hard, Spec: 2 gaps). Fixed in two follow-up commits (hub, then client), each TDD-gated:

- **I-M1 fixed (hub F1).** `Continue` overflow re-recorded the refused tail with the `/continue` id. `Accept` no longer records refusals for `SpawnReason.Continuation`; `Continue` re-records every replayed id that did not make it into `Pending` with its ORIGINAL refusing id. Test: three refused (`a`, `b`, `c`), `/continue turns: 1` → `Pending` = `a` only, `Refused` = `b`, `c` with their original ids; a second `/continue` queues `b` with `RefusedAt` = its original id.
- **I-M2 fixed (hub F2).** `LastModelPost` is set only when the author is spawnable (`IsSpawnable(author)`), so an app-backed `claude`/`codex` post never buys a synthesis. Test: `claude` posts into an open exchange after the addressee's post; `Finished` concludes with no synthesis.
- **I-M3 fixed (hub F3 + client C1).** A malformed leading `turns:` token (`turns: 1000`, `turns:  3`, `turns:16x`) no longer breaks the walk at position 0. Both readers consume any `turns:` token in the run: `[Tt][Uu][Rr][Nn][Ss]:[ \t]*(?<n>[0-9]*)(?<junk>[A-Za-z0-9_.\-]*)`; digits only and 1..16 → `Valid`; anything else (empty, junk, out of range) → `OutOfRange` with value 0 on BOTH sides (the TS shape converges: `{ turns: 0, valid: false }`). D-b's "not followed by a letter" clause is replaced by this: a wrong token is noted, never silently prose, and the message still dispatches. Fixture gains `turns: 1000 @opus go` and `turns:16x @opus go` (out-of-range, recipients `opus`), `turns:  3 @opus go` (valid 3: `[ \t]*` tolerates the double space), `turns: 16 @opus` (valid 16) and `turns: 17 @opus` (out-of-range) so the ceiling is pinned on both sides; out-of-range cases assert value 0. `ExchangeCommandsTests` trailing-position cases become `@opus turns:  3` valid 3 and `@opus turns: 3x` out-of-range, not none.
- **I-M4 fixed (client C2).** The strip's preview line no longer claims a set: a `/continue` draft says `Adds N turns.`, any other draft says `Asks for N turns.` (a reply that joins an open exchange keeps its turns; the hub, not the strip, decides). The out-of-range chip text is the hub's own note verbatim: `turns: must be a whole number from 1 to 16; the default 8 applies.`
- **I-M5 resolved outside the patch.** AC8 ran this session: `.claude/skills/verify-chopitup/helpers/Invoke-ContinueCapture.ps1` (gitignored by design) 28/28, three captures sane, judged 3/3.
- **I-m1 fixed (hub F4).** `Continuable` is false while the room's latest run is parked (a `/continue` there would resume the run). Predicate moved to `ExchangePolicy.Continuable(Exchange x, bool runBlocks)` (Standards S3), used by `View` and the top level; `runBlocks` = active OR parked run in the room.
- **I-m2 fixed (hub F5).** Dry-run leg 4 asserts the entry's `inFlight`/`pending` unchanged and the exchange count unchanged after the refused `/continue`, not `budget -eq 2`.
- **I-m3 partly fixed (hub F6).** The unknown-word notes in `Continue` move before the `RunExchange`/`StillOpen`/`StillFinishing` guards, so `/continue @sonet` on an open exchange still gets its row 43 note. A refused `/continue` is not made a member of the exchange (declined: it did nothing to it).
- **I-m4 fixed by C2** (wording) and by the fixture boundary cases (F3/C1); the constants stay duplicated per D-a.
- **I-m5 declined.** `recipient-chip turns` renders in the accent colour and was judged fine in the AC8 captures; the class is a hook.
- **I-m6 + Spec c3 fixed (hub F7).** `PendingSpawn`/`SpawnRequest` gain `long? RefusedAt` (set by `Continue` for a replayed hand-off) and `SpawnPromptInput` gains `RefusedAt` and `LastModelPost` (`(string AuthorId, long MessageId)?`, the exchange's, passed by the service). The synthesis why-line names `LastModelPost` (author and id), never `TriggerIds[^1]`; the replayed-continuation why-line fires on `RefusedAt is not null` (citing it), never on trigger count; `LastPoster` is deleted. A debounce-merged owner reply can no longer change either sentence. Prompt tests updated to the new inputs; R14 golden untouched.
- **I-m7 fixed (hub F8).** `DropQueuedSynthesis` takes back `TurnsCommitted` with `Budget` when the synthesis had grown it, so the wire never shows `turnsCommitted > budget`.
- **I-m8 fixed (hub F9).** `Due` builds a `Synthesis` request with `RemainingAfter = 0` (the exchange concludes after it unconditionally).
- **Standards S1 fixed (hub F10 + client C3).** The 200 ms bound applies to the perf-canary case only (name starts `perf canary`), not to every fixture case.
- **Standards S2 fixed** by F3/C1 (boundary cases, converged out-of-range shape).
- **Standards S3 fixed** by F4.
- **Standards judgement calls:** unknown-word note loop and the turns-or-default read extracted into two private helpers used by `OnRoomMessage` and `Continue` (hub F11); `SkillImport` reserved-name pair, the `Kind == "human"` reads, the defaulted parameters and `Exchange`'s field cluster declined (house shapes; the defaults are what keep old positional call sites compiling).
- **Spec a1 (AC8)** = I-M5. **Spec a2 fixed (hub F12).** The range note posts only when an exchange opens (after the `mentioned.Count == 0` return); a `turns: 99` on a message that opens nothing is prose to the hub. A joining reply's token is ignored without a note (the strip's `Asks for` wording covers it).
