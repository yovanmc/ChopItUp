...
- **I-M3 fixed (hub F3 + client C1).** A malformed leading `turns:` token (`turns: 1000`, `turns:  3`, `turns:16x`) no longer breaks the walk at position 0. Both readers consume any `turns:` token in the run: `[Tt][Uu][Rr][Nn][Ss]:[ \t]*(?<n>[0-9]*)(?<junk>[A-Za-z0-9_.\-]*)`; digits only and 1..16 → `Valid`; anything else (empty, junk, out of range) → `OutOfRange` with value 0 on BOTH sides (the TS shape converges: `{ turns: 0, valid: false }`). D-b's "not followed by a letter" clause is replaced by this: a wrong token is noted, never silently prose, and the message still dispatches. Fixture gains `turns: 1000 @opus go`, `turns:  3 @opus go`, `turns:16x @opus go` (all out-of-range, recipients `opus`), `turns: 16 @opus` (valid 16) and `turns: 17 @opus` (out-of-range) so the ceiling is pinned on both sides; out-of-range cases assert value 0. `ExchangeCommandsTests` trailing-position cases (`@opus turns:  3`, `@opus turns: 3x`) become out-of-range, not none.
