...
| 6 | `Participant` is `(Id, DisplayName, Kind, Host, Model, Note, Classes = null)` — `src/ChopItUp.Core/Model/Message.cs:36`; `SeedRoster` calls it positionally with 6 or 7 args, so a new param must be LAST and optional | ddfa572 | `pwsh -NoProfile -Command "(Select-String -Path src/ChopItUp.Core/Model/Message.cs -Pattern 'Classes = null').Count"` must print 1 |
...
| 8 | `BearerTokenMiddleware` guards by METHOD not by route: every non-GET/HEAD/OPTIONS `/api` request needs an owner-class bearer, so a new write route is owner-only with no extra attribute | ddfa572 | `pwsh -NoProfile -Command "(Select-String -Path src/ChopItUp.Hub/Security/BearerTokenMiddleware.cs -Pattern 'HttpMethods.IsGet').Count"` must print 1 or more. This pins the by-method expression rather than the mere existence of a method name; if it ever stops matching, read the method and pin whatever expression implements the GET/HEAD/OPTIONS exemption, and do not weaken it back to a name. |
...
| 12 | Codex has NO `--append-system-prompt` equivalent; both hosts take the whole prompt on stdin, so role/persona text must live in the rendered prompt string, not a CLI flag (`SpawnPrompt.cs:141-149`, `SpawnCommands.cs:66-76,223-236`) | ddfa572 | `pwsh -NoProfile -Command "(Select-String -Path src/ChopItUp.Hub/Spawning/SpawnPrompt.cs -Pattern 'and Codex has$').Count"` must print 1. The sentence wraps across lines 142 and 143, so a pattern spelling out Codex-has-none fails on a claim that is true. |
