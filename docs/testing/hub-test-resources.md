...
|---|----------|-------|-------|---------|
| R1 | SQLite connection pools, cleared by `SqliteConnection.ClearAllPools()`. Inert: every connection in `src/` and these tests sets `Pooling=false` (`ChopDb.cs:65`, `:159`, `HostCommands.cs:47`), so no pool exists to clear and R1 serializes nothing | process | `HubTestHost.cs:120` (every host dispose), `DryRunTests.cs:44`, `HostCommandsTests.cs:37`, `:83`, `:484`, `Skills/ConsolidateMemorySkillTests.cs:42`, `Skills/SkillImportTests.cs:32`, `Spawning/SpawnerServiceTests.cs:838` | every host user (R4) plus DryRunTests, HostCommandsTests, ConsolidateMemorySkillTests, SkillImportTests, SpawnerTimingTests |
| R2 | Environment variables | process | `HubHostTests.cs:390`, `:397` (`CHOPITUP_SHELL_TOKEN`), `Spawning/PanelProcessTests.cs:29-30`, `:42-43` (synthetic sentinel names). Production code writes it too: `HubHost.Build` clears `CHOPITUP_SHELL_TOKEN` on every host build (`src/ChopItUp.Hub/Hosting/HubHost.cs:71`), so every R4 class writes R2; that is safe only while HubHostTests stays in the non-parallel collection | HubHostTests, PanelProcessTests (writers); every R4 class through `HubHost.Build` |
| R3 | `Console.Error` redirected by `Console.SetError` | process | `HubHostTests.cs:227`, `:271`, `:322`, `:337`, `Skills/SkillStoreTests.cs:194`, `Spawning/SpawnerShutdownConsoleTests.cs:45` | HubHostTests, SkillStoreTests, SpawnerShutdownConsoleTests |
...
| R5 | Fixed loopback port: reserved with a port-0 `TcpListener`, released, then bound by the host | machine | `Security/OwnerPeerCheckEndToEndTests.cs:35-38` | OwnerPeerCheckEndToEndTests |
| R6 | Machine TCP table and process table (WMI `Win32_Process`) read and matched by pid | machine | `Security/PeerProcessTests.cs:45`, `Spawning/ProcessRunnerTests.cs:58`, `Spawning/SpawnJobsTests.cs:42`, `Security/OwnerPeerCheckEndToEndTests.cs:117` | PeerProcessTests, ProcessRunnerTests, SpawnJobsTests, OwnerPeerCheckEndToEndTests |
| R7 | Named kernel mutex `Global\ChopItUp.Skills.<path>` | machine, name unique per test directory | `Skills/SkillImportTests.cs:793`, `:800` | SkillImportTests |
| R8 | Real child processes (git, pwsh, cmd, ping, mklink) confined to the test's directory | machine process list, per-test files | `DeployScriptTests.cs:160`, `GateScriptTests.cs:202`, `Git/GitTrailTests.cs:23`, `Memory/MemoryGitTests.cs:34`, `MemoryEditApiTests.cs:70`, `RoomsApiTests.cs:289`, `Rooms/ExchangeWorktreesTests.cs:55`, `Rooms/RoomPathsTests.cs:89`, `Skills/SkillImportTests.cs:78`, `Spawning/PanelInputsTests.cs:109`, `Spawning/PanelProcessTests.cs:16`, `Spawning/SpawnerServiceTestBase.cs:160`, `Security/OwnerPeerCheckTests.cs:28`, plus the R6 classes | see the class table |
...
| R11 | Process-wide statics in production code: `MemoryApi.Decisions` and `SkillsApi.Decisions` (`SemaphoreSlim(1, 1)`), `MemoryApi.RefusalNoted` keyed by data root and id, `SkillsApi.TreeCacheByDir` keyed by full path and cleared past 200 entries | process | `src/ChopItUp.Hub/Web/MemoryApi.cs:22`, `:29`, `src/ChopItUp.Hub/Web/SkillsApi.cs:35`, `:47` | MemoryApiTests, MemoryApiGuardTests, MemoryEditApiTests, SkillsApiTests, SkillsApiAuthTests, SkillsApiProposalsTests. Concurrent hosts queue on the two semaphores (a wait, not a correctness hazard); the keyed caches are per directory |

Everything else a test writes goes under its own unique temp directory, deleted by `TestDirs.DeleteTree` (`TestDirs.cs:8`). `DeployScriptTests` names the real deploy directory (`DeployScriptTests.cs:498`) and documents at `:757` that the test keeps the script's backup path out of it.
...
- R6: `SpawnJobsTests.cs:28-31` documents a conhost job-membership race seen on isolated runs. Tests that count processes by parent pid can see processes another test started.
...
- Resources reached through production code the tests call, beyond the environment write in R2 and the statics in R11. The audit read the test project, the helpers it owns and those two production sites.
